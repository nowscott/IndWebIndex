...
const database = vi.hoisted(() => {
  ...
  return { state, createTurso, turso: createTurso() };
});
...
beforeEach(() => {
  ...
  database.turso = database.createTurso();
...
describe('visit count API', () => {
  it('records background page views and exposes them to the build snapshot', async () => {
    ...
    await Promise.all([
      handler(postRequest, first),
      handler(postRequest, second),
    ]);
    ...
    expect(database.turso.batch).toHaveBeenCalledTimes(3);
  });

  it('retries table setup after a transient initialization failure', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
    database.turso.batch
      .mockRejectedValueOnce(new Error('temporary setup failure'))
      .mockImplementation(async statements => {
        for (const statement of statements) await database.turso.execute(statement);
        return [];
      });
