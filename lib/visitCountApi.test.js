...
vi.mock('./turso', () => ({ turso: database.turso }));
...
describe('visit count API', () => {
  it('records every page load as a real view and keeps GET read-only', async () => {
    const first = createResponse();
    await handler(postRequest, first);
    const second = createResponse();
    await handler(postRequest, second);
    const get = createResponse();
    await handler({ method: 'GET', headers: { host: 'example.test' } }, get);

    const today = getShanghaiDate();
    expect(first.body).toMatchObject({ enabled: true, count: 1, startedAt: today });
    expect(second.body.count).toBe(2);
    expect(get.body.count).toBe(2);
    expect(get.body.activity.find(day => day.date === today)).toEqual({
      date: today,
      count: 2,
      estimated: false,
    });
