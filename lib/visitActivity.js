...
export const getActivityLevelMaximum = counts => {
  ...
  return sortedCounts[Math.ceil(sortedCounts.length * 0.95) - 1];
};

export const getActivityLevel = (count, maximum) => {
  if (!count) return 0;
  return Math.min(4, Math.max(1, Math.ceil((Math.min(count, maximum) / maximum) * 4)));
};
