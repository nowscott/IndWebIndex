...
import { formatNotionPages, getAllPages, getDatabase } from './notion';
...
beforeEach(() => {
  ...
  delete process.env.SKIP_NOTION_FETCH;
  vi.restoreAllMocks();
});
...
describe('getDatabase', () => {
  it('retries rate limits and transient server errors', async () => {
    const rateLimited = { code: 'rate_limited', headers: { get: () => '0' } };
    const fetchPage = vi.fn()
      .mockRejectedValueOnce(rateLimited)
      .mockRejectedValueOnce({ status: 503 })
      .mockResolvedValueOnce({ results: [{ id: 'valid' }], has_more: false });

    await expect(getAllPages('database-a', fetchPage)).resolves.toEqual([{ id: 'valid' }]);
    expect(fetchPage).toHaveBeenCalledTimes(3);
  });

  it('does not retry non-transient Notion errors', async () => {
    const fetchPage = vi.fn().mockRejectedValue({ status: 401 });

    await expect(getAllPages('database-a', fetchPage)).rejects.toMatchObject({ status: 401 });
    expect(fetchPage).toHaveBeenCalledTimes(1);
  });

  it('supports explicit build-time Notion fetch skipping', async () => {
    process.env.SKIP_NOTION_FETCH = '1';

    await expect(getDatabase(undefined, vi.fn())).resolves.toEqual([]);
  });
