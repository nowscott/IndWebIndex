...
// 切换标签按钮
export const toggleTagButton = (tag, onList, setOnList) => {
  const newOnList = _.xor(onList, [tag]);
  setOnList(newOnList);
};

// 更新结果
export const updateResults = (posts, onList, setFilteredPosts, setTags, referenceTags) => {
  ...
  // Use referenceTags to maintain the ISR-randomized order if provided
  let sortedTags;
  if (referenceTags && referenceTags.length > 0) {
    const sortedAvailableTags = referenceTags.filter(tag => availableTags.includes(tag) && !onList.includes(tag));
    sortedTags = onList.concat(sortedAvailableTags);
  } else {
    sortedTags = onList.concat(availableTags.filter(tag => !onList.includes(tag)));
  }
  setTags(sortedTags);
};