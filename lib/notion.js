import { Client } from '@notionhq/client';

// 使用 global 来持久化缓存，防止在开发模式下因热更新导致缓存失效
const globalCache = global.notionCache || {
  data: null,
  lastFetched: null,
};

if (process.env.NODE_ENV !== 'production') {
  global.notionCache = globalCache;
}

const CACHE_TTL = 30 * 60 * 1000; // 缓存 30 分钟

// 创建 Notion 客户端实例，使用环境变量中的 NOTION_TOKEN 进行身份验证
const getNotionClient = () => {
  return new Client({ auth: process.env.NOTION_TOKEN });
};

// 获取指定数据库的所有页面
export const getAllPages = async (databaseId) => {
  if (!databaseId) {
    console.error('DATABASE_ID is missing');
    return [];
  }
  
  const notion = getNotionClient();
  let results = []; // 存储所有页面的数组
  let hasMore = true; // 用于迭代查询，检查是否还有更多页面需要获取
  let startCursor = undefined; // 用于获取下一页的游标

  try {
    // 当仍有更多页面需要获取时循环执行
    while (hasMore) {
      // 查询数据库的一页数据
      const response = await notion.databases.query({
        database_id: databaseId,
        start_cursor: startCursor,
      });

      // 将当前页的结果追加到结果数组中
      results = results.concat(response.results);
      // 更新是否还有更多页面的标志
      hasMore = response.has_more;
      // 更新下一页的游标，以便下次查询时获取下一页数据
      startCursor = response.next_cursor;
    }
  } catch (error) {
    console.error('Error fetching pages from Notion:', error);
    return [];
  }

  return results; // 返回所有页面的数组
};

// 获取指定数据库的页面并转换为适合显示的格式
export const getDatabase = async (databaseId) => {
  const now = Date.now();
  
  // 如果缓存存在且未过期，直接返回缓存数据
  if (globalCache.data && globalCache.lastFetched && (now - globalCache.lastFetched < CACHE_TTL)) {
    console.log('Using Notion database cache...');
    return globalCache.data;
  }

  console.log('Fetching fresh data from Notion...');
  // 获取指定数据库的所有页面
  const pages = await getAllPages(databaseId);
  // 将页面格式化为适合显示的格式
  const formattedData = pages
    .filter(page => page.properties.empty.formula.string !== 'empty') // 过滤掉空的页面
    .filter(page => page.properties.state.select.name !== '断开连接') // 过滤掉状态为 "断开连接" 的页面
    .map(page => {
      const properties = page.properties;
      return {
        id: page.id, // 页面的 ID
        name: properties.name.title[0].plain_text, // 页面的名称
        state: properties.state.select.name, // 页面的状态
        brief: properties.brief.rich_text[0].plain_text, // 页面的简介
        tags: properties.tags.multi_select.map(tag => tag.name), // 页面的标签数组
        web: properties.web.url, // 页面的网址
      };
    });

  // 更新缓存
  globalCache.data = formattedData;
  globalCache.lastFetched = now;

  return formattedData;
};