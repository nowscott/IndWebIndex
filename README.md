...
目前的 **3.x** 版本，正式实现了这一构想。我们利用 Notion API 将数据彻底云端化，并配合全新的前端架构，实现了极致的访问速度。这意味着，我只需在手机或电脑的 Notion App 里随手一记，几秒钟后，我的个人黄页就会自动更新。
...
> **参考资源**：[用 Notion 当数据库写一个简单的 API][bilibili-url]
...
为了不让等待消磨掉寻找灵感的兴致，在 **3.x** 版本中，我们实施了深度的技术优化，通过所谓的“双层缓存机制”彻底解决了这一瓶颈：
...
### 📊 访问统计 (可选)
如果你还希望开启**全站访问量统计和访问活动热力图**，只需在 [Turso][turso-url] 创建一个免费数据库，并配置 `TURSO_DATABASE_URL` 与 `TURSO_AUTH_TOKEN`。浏览会在后台写入；关于页使用 30 分钟 ISR 快照更新，与首页及 Notion 数据缓存周期一致，因此刷新页面不会等待统计接口。首次访问会自动建表；热力图从此开始记录真实浏览，未统计的历史仅以“估算”形式展示。若未配置，该功能将静默隐藏，绝不打扰。

### 🛠 本地维护
仓库提供了 `.env.example`，可以先复制为 `.env.local`，再填入自己的 Notion 与 Turso 配置。本地常用验证命令如下：
...
如果只是验证 CI 或构建链路、暂时不需要连接真实 Notion 数据库，可以临时设置 `SKIP_NOTION_FETCH=1`。这个开关只建议用于测试构建，不建议用于正式部署。

## 🧭 维护计划

后续更新会优先围绕可靠性、数据质量、搜索性能和可访问性展开。未来版本规划与版本历史见 [CHANGELOG.md](./CHANGELOG.md)。
