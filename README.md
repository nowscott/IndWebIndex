...
### 📊 访问统计 (可选)
如果你还希望开启**全站访问量统计和访问活动热力图**，只需在 [Turso][turso-url] 创建一个免费数据库，并配置 `TURSO_DATABASE_URL` 与 `TURSO_AUTH_TOKEN`。首次访问会自动建表；热力图从此开始记录真实浏览，未统计的历史仅以“估算”形式展示。若未配置，该功能将静默隐藏，绝不打扰。
